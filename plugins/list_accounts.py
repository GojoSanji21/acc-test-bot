...
async def bulk_export_handler(callback_query: CallbackQuery):
    ...
    if action == "sqlite":
        await callback_query.message.edit_text(
            f"⏳ <b>ᴄᴏɴᴠᴇʀᴛɪɴɢ {len(accounts)} sᴇssɪᴏɴs to SQLite <code>.session</code> databases... ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ.</b>",
            parse_mode="HTML"
        )
        ...
        try:
            ...
            if zip_path.exists() and zip_path.stat().st_size > 0:
                await callback_query.bot.send_document(
                    ...
                    caption=f"📦 <b>ʙᴜʟᴋ SQLite sᴇssɪᴏɴs (.ᴢɪᴘ) ᴇxᴘᴏʀᴛᴇᴅ sᴜᴄᴄᴇssꜰᴜʟʟʏ!</b>\n\n📱 <b>ᴛᴏᴛᴀʟ ᴀᴄᴄᴏᴜɴᴛs:</b> <code>{len(accounts)}</code>",
                    parse_mode="HTML"
                )
                await callback_query.message.edit_text(
                    f"✅ <b> SQLite sᴇssɪᴏɴs ZIP file sent successfully!</b>\n\n📱 <b>ᴛᴏᴛᴀʟ ᴀᴄᴄᴏᴜɴᴛs:</b> <code>{len(accounts)}</code>",
                    reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                        [InlineKeyboardButton(text="🔙 ʙᴀᴄᴋ ᴛᴏ ʟɪsᴛ", callback_data=f"back_to_list:{page}")]
                    ]),
                    parse_mode="HTML"
